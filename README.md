...
| `Hetzner Database Server makepad` | `staging-brio-identity-db` and `keycloak-cohort-restore` | the same five native SSH fields map to the workflow aliases `BRIO_IDENTITY_DB_DEPLOY_SSH_*` and `KEYCLOAK_COHORT_DB_SSH_*` only in their named standalone DB-host environments |
