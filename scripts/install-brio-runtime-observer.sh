...
[[ $(wc -c < "${control_source}") -le 131072 ]] || die 'Control-helper source is unexpectedly large.'
...
for controlled_path in "${observer_command}" "${control_command}" "${sudoers_path}" "${observer_home}" \
...
[[ ! -e "${control_command}" || -f "${control_command}" ]] || die 'Control-helper path has an unsafe file type.'
